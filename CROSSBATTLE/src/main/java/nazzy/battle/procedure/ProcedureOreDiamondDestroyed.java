...
public class ProcedureOreDiamondDestroyed {
    ...
    public static void executeProcedure(HashMap<String, Object> dependencies) {
        if (dependencies.get("entity") == null || dependencies.get("x") == null ||
            ...
            System.err.println("Failed to load dependencies for procedure OreDiamondDestroyed!");
            return;
        }
        ...
        if (entity instanceof Player player) {
            ItemStack stack = new ItemStack(InitBlocks.ORE_DIAMOND.get().asItem(), dropCount);
            player.getInventory().add(stack);

            if (!world.isClientSide()) {
                player.displayClientMessage(Component.literal("§l§a+" + dropCount + " §bАлмаз"), true);
            }
        } else if (!world.isClientSide()) {
            world.addFreshEntity(new ItemEntity(world, x, y + 0.5, z,
                    new ItemStack(InitBlocks.ORE_DIAMOND.get().asItem(), dropCount)));
        }
