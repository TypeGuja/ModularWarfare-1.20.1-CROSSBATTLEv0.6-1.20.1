...
public class ProcedureOreIronDestroyed {
    ...
    public static void executeProcedure(HashMap<String, Object> dependencies) {
        if (dependencies.get("entity") == null || dependencies.get("x") == null ||
            ...
            System.err.println("Failed to load dependencies for procedure OreIronDestroyed!");
            return;
        }
        ...
        if (entity instanceof Player player) {
            ItemStack stack = new ItemStack(InitBlocks.ORE_IRON.get().asItem(), dropCount);
            if (!player.getInventory().add(stack)) {
                player.drop(stack, false);
            }

            if (!world.isClientSide()) {
                player.displayClientMessage(Component.literal("§l§a+" + dropCount + " §6Железо"), true);
            }
        } else if (!world.isClientSide()) {
            world.addFreshEntity(new ItemEntity(world, x, y + 0.5, z,
                    new ItemStack(InitBlocks.ORE_IRON.get().asItem(), dropCount)));
        }
